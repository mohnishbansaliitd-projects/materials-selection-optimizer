"""Component-level material selection for the gearbox case study: gear, shaft, housing."""
...
def analyze_gear_material(df: pd.DataFrame) -> Dict[str, Any]:
    """Spur gear/pinion: driven by Hertz contact fatigue and Lewis root bending strength."""
...
def analyze_shaft_material(df: pd.DataFrame) -> Dict[str, Any]:
    """Transmission shaft: torsional strength and stiffness under fatigue loading."""
    screener = MaterialScreener(df)
    screened, stats = screener.filter(
...
def analyze_housing_material(df: pd.DataFrame) -> Dict[str, Any]:
    """Gearbox housing: panel bending rigidity (E^(1/3)/rho) traded against mass and cost."""
    screener = MaterialScreener(df)
    screened, stats = screener.filter(
